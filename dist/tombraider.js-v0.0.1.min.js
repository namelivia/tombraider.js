(() => {
  ...
  var TombRaiderMenu = function(containerId) {
    ...
    this.camera = new window.THREE.PerspectiveCamera(50, ww / hh, 1, 1e5);
    this.renderer = new window.THREE.WebGLRenderer({alpha: true});
    this.renderer.setSize(ww, hh);
    container.appendChild(this.renderer.domElement);
    var ambientLight = new window.THREE.AmbientLight(16637848);
    var directionalLight = new window.THREE.DirectionalLight(16777215);
  ...
  TombRaiderMenu.prototype.newScene = function() {
    ...
    this.angleStep = 0;
    this.setRadius(6e3);
    this.setCameraDistance(12e3);
    this.setCameraHeight(3e3);
  ...
  TombRaiderMenu.prototype.addModel = function(name, url, action, params) {
    var newModel = {name, model: url, action, params};
  ...
  TombRaiderMenu.prototype.update = function() {
    ...
    if (this.cameraAngle !== this.angleStep * this.selectedRotate) {
      if (this.cameraAngle < this.angleStep * this.selectedRotate) {
        this.cameraAngle = this.cameraAngle + (this.angleStep * this.selectedRotate - this.cameraAngle) / 10;
      } else {
  ...
  TombRaiderMenu.prototype.setConfig = function(config) {
    ...
    for (var i = 0; i < objectsLength; i++) {
      this.addModel(config.objects[i].name, config.objects[i].model, config.objects[i].action, config.objects[i].params);
    }
  };
